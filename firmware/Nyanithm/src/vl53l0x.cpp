...
#define startTimeout() (timeout_start_ms = to_ms_since_boot(get_absolute_time()))

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && ((uint16_t)(to_ms_since_boot(get_absolute_time()) - timeout_start_ms) > io_timeout))
...
// VL53L0X::VL53L0X()
//     : bus(&Wire), address(ADDRESS_DEFAULT), io_timeout(0)  // no timeout
//       ,
//       did_timeout(false) {}

VL53L0X::VL53L0X(uint8_t _port, uint8_t i2c_addr) {
    port = _port;
    address = i2c_addr;
}
...
bool VL53L0X::init(bool io_2v8) {
    ...
    // VL53L0X_PerformRefCalibrationa() end
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value) {
    ...
    int r0 = i2c_write(port, address, buf, 2, false);
    // bus->beginTransmission(address);hhh
    // bus->write(reg);
    // int r1 = i2c_write(port, address, &value, 1, false);
    // bus->write(value);
...
/**
 * should be call in your loop as a task
 */
...
uint64_t VL53L0X::getAsyncMeasureTime() {
    return measureTime;
}
...
bool VL53L0X::performSingleRefCalibration(uint8_t vhv_init_byte) {
    writeReg(SYSRANGE_START, 0x01 | vhv_init_byte);  // VL53Lh0X_REG_SYSRANGE_MODE_START_STOP
